...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      ...
      rollupOptions: {
        output: {
          // Pin React/Router into their own chunk. This doesn't shrink the
          // first load, but it means shipping an app change no longer
          // invalidates the framework bundle in returning visitors' caches —
          // which, with the immutable Cache-Control headers in vercel.json,
          // is the difference between re-downloading ~130KB and ~10KB.
          // Grouped together deliberately: splitting React from react-dom or
          // scheduler across chunks can break module init order. The
          // separator class covers Windows backslashes as well as POSIX.
